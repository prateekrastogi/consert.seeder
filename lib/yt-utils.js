...
exports.searchYtVideos = function searchYtVideos (queries, maxresults, params) {
  ...
  const youtube = eventType ? loginAssist.ytLiveLogin() : loginAssist.ytLogin()

  const queryObservable = Rx.Observable.from(queries)

  const ytVideos = queryObservable.mergeMap(query => searchYt(query, maxresults, params, youtube).filter(result => eventType ? true : filterVideoByTitle(result)))
