...
exports.convertMediaItemToRecombeeItem = function convertMediaItemToRecombeeItem (video) {
  const recombeeVideo = {
    'itemType': 'video',
    'kind': video.kind,
    'etag': video.etag,
    ...
    'contentDetails-regionRestriction': video.contentDetails.regionRestriction,
    'contentDetails-contentRating': video.contentDetails.contentRating,
    'contentDetails-projection': video.contentDetails.projection,
    'contentDetails-hasCustomThumbnail': video.contentDetails.hasCustomThumbnail,
    ...
    'snippet-tags': video.snippet.tags,
    'snippet-categoryId': video.snippet.categoryId,
    'snippet-liveBroadcastContent': video.snippet.liveBroadcastContent,
    'snippet-defaultLanguage': video.snippet.defaultLanguage,
    'snippet-localized': video.snippet.localized,
    'snippet-defaultAudioLanguage': video.snippet.defaultAudioLanguage,
    'liveStreamingDetails-actualStartTime': video.liveStreamingDetails ? video.liveStreamingDetails.actualStartTime : null,
    'liveStreamingDetails-actualEndTime': video.liveStreamingDetails ? video.liveStreamingDetails.actualEndTime : null,
    'liveStreamingDetails-scheduledStartTime': video.liveStreamingDetails ? video.liveStreamingDetails.scheduledStartTime : null,
    'liveStreamingDetails-scheduledEndTime': video.liveStreamingDetails ? video.liveStreamingDetails.scheduledEndTime : null,
    'liveStreamingDetails-concurrentViewers': video.liveStreamingDetails ? video.liveStreamingDetails.concurrentViewers : null,
    'liveStreamingDetails-activeLiveChatId': video.liveStreamingDetails ? video.liveStreamingDetails.activeLiveChatId : null,
    ...
    const dbUpdateRequest = Rx.Observable.fromPromise(model.findById(id)).map(item => {
      switch (model.modelName) {
        ...
        case 'ytVideo':
          ...
          break
        case 'ytBroadcast':
          item.isBroadcastRecombeeSynced = true
          break
      }
      return item
