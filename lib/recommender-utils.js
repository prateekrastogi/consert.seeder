...
const { from, concat, EMPTY } = require('rxjs')
...
exports.writeBufferedItemsToRecommender = function writeBufferedItemsToRecommender (bufferedItems, model) {
  ...
  const dbUpdateBatchRequest = from(ids).pipe(
    concatMap(id => {
      const dbUpdateRequest = from(model.findById ? model.findById(id) /* For genre items */ : EMPTY).pipe(
        map(item => {
  ...
  //  const genreDataJSONUpdator = bindNodeCallback(fs.writeFile)('lib/genreData.json', JSON.stringify(genreDataObject, null, 2), 'utf8')
