...
const durationBinEdges = []

const dateTimeBinEdges = []

const viewCountBinEdges = []
const likeCountBinEdges = []
const dislikeCountBinEdges = []
const favoriteCountBinEdges = []
const commentCountBinEdges = []

const artistFollowersBinEdges = [10, 100, 1000, 10000, 100000]
...
exports.convertMediaItemToRecommenderMediaItem = function convertMediaItemToRecommenderMediaItem (mediaItem) {
  const recommenderMediaItem = {
    ...
    'liveStreamingDetails-concurrentViewers': mediaItem.liveStreamingDetails ? mediaItem.liveStreamingDetails.concurrentViewers : [],
...
function binningDelegate (jsonFieldName, dataPoint, [...binEdges]) {
  switch (jsonFieldName) {
    case 'liveStreamingDetails-actualStartTime':
      return binDateTime(dataPoint, binEdges)
    case 'contentDetails-duration':
      return binDuration(dataPoint, binEdges)
    default:
      return R.is(Number, dataPoint) ? binNumber(dataPoint, binEdges) : R.map(binNumber(R.__, binEdges), [...dataPoint])
  }
  ...
  function binDateTime (dataPoint, [...binEdges]) {}
}
