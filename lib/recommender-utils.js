...
const { concatMap, map, retry, tap } = require('rxjs/operators')
...
exports.writeBufferedItemsToRecommender = function writeBufferedItemsToRecommender (bufferedItems, model) {
  const itemPropertyAddBatchRequest = from(bufferedItems).pipe(
    map(({ id, recommenderItem}) => {
      const recommenderItem = {
        iid: id,
        properties: {
          ...itemProperties
        }
    ...
    tap(recommenderItem => console.log(recommenderItem)),
    concatMap(recommenderItem => from(predictionioClient.createItem(recommenderItem))),
    retry(RETRY_COUNT)
  )
